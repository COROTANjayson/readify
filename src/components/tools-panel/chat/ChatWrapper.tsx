  ...
  const { data, isLoading } = trpc.file.getFileUploadStatus.useQuery(
    ...
    },
    {
      refetchInterval: (query: any) => {
        const status =
          query?.state?.data?.status === UploadStatus.SUCCESS || query?.state?.data?.status === "FAILED" ? false : 500;
        return status;
      },
    }
  );
  ...
  if (isLoading)
    return (
      <div className="relative min-h-full bg-zinc-50 flex divide-y divide-zinc-200 flex-col justify-between gap-2">
        <div className="flex-1 flex justify-center items-center flex-col mb-32">
